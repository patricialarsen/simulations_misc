...
int main(int argc, char *argv[]) {
  ...
  if(argc != 13) {
     if (commRank==0){
     fprintf(stderr,"USAGE: %s <inputfile> <inputfilehalo> <outputfile> <nside> <nside_low> <step> <hval> <samplerate> <start_step> <nsteps> <masscut> <radiuscut> \n", argv[0]);
     }
     exit(-1);
  }
  ...
  const char *mpiioName_base2 = filename2;
  ...
  float masscut = atof(argv[11]);
  float radiuscut = atof(argv[12]);
  ...
  int status = assign_sz_cic_halocut(rho, phi, vel,ksz,tsz, &P, &H, map_hires, pix_nums_start, pix_nums_end,start_idx, ring_to_idx, hval, borgcube, adiabatic,  samplerate, cloudypath, masscut, radiuscut);
